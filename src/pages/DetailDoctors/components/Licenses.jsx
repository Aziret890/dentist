  ...
  return (
    <section id="section3" className="mb-20">
      ...
      <Swiper
        ...
        spaceBetween={10}
        slidesPerView={width < 1100 && width > 830 ? 3 : width < 830 && width > 550 ? 2 : width < 550 ? 1 : 4}
        keyboard
      ...
      >
        {doc?.certificate &&
          doc?.certificate?.map((item, idx) => (
            <SwiperSlide key={idx}>
              <li onClick={() => setCertificateId(idx)} data-aos="fade-up" data-aos-duration={idx * 500}>
                <div className={styles['licenses-image']}>
                  <img src={'https://akmatovt.pythonanywhere.com/media/' + item.image} alt="" />
                </div>
                <h4>{item.title}</h4>
              </li>
            </SwiperSlide>
